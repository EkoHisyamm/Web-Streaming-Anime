...
$lenght = mysqli_num_rows(mysqli_query($con, 'SELECT * FROM `movies`'));
$search = $_GET['search'];
...
switch (isset($_GET)) {
  case ($_GET['page1']):
    ...
    if (!empty($search)) {
      ...
      $lenght = mysqli_num_rows(mysqli_query($con, 'SELECT * FROM movies WHERE `judul` LIKE "%' . $search . '%"'));
    }
    break;
  default:
              ...
              <tbody>
                ...
                ?>
                  <tr>
                    ...
                    <td>
                      <a href="editmoviec.php?id=<?php echo $row['id'] . '-' . $current ?>" name="edit" title='Update Record' data-toggle='tooltip'><span class='fas fa-edit'></span></a>
